...
export class Server {
  ...
  protected basicAuth(req: http.IncomingMessage) {
    ...
    const decode = (auth: string) => {
      return Buffer.from(auth.split(" ")[ 1 ], "base64").toString().split(":");
    };
  ...
  async doRequest(req: http.IncomingMessage, res: http.ServerResponse) {
    if (this.hostname(req.headers.host) === this.config.domain) {
      ...
      if (tunnel) {
        this.proxy.web(req, res, tunnel.getWebOptions(this.config));
      } else {
        res.writeHead(502);
        res.end("Bad Gateway");
      }
  ...
  protected doUpgrade(req, socket, head) {
    if (this.hostname(req.headers.host) === this.config.domain) {
      ...
      let tunnel = this.getTunnelByHost(req.headers.host);
      if (tunnel) {
        this.proxy.ws(req, socket, head, tunnel.getWsOptions(this.config));
      } else {
        socket.destroy();
      }
    }
  ...
  async doRegister(subdomain: string, username, options) {
    let tunnel = this.manager.getTunnel(subdomain);
    if (tunnel) {
      throw new Error("already connected");
    }
    tunnel = await this.manager.newTunnel(subdomain, username, options);
  ...
  async route(req: http.IncomingMessage) {
    ...
    for (const r of this.routes) {
      let matched = pathname.match(r);
      if (matched) {
        let [full, ...params] = matched;
        if (r.meta[ method ]) {
          return await r.meta[ method ](...params, this.basicAuth(req), options);
        }
      }
    }
  }

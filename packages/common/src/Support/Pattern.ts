export class Pattern<T> extends RegExp {
    /**
...
function tokensToFunction(tokens) {
    ...
    for (var i = 0; i < tokens.length; i++) {
        if (typeof tokens[i] === 'object') {
            matches[i] = new Pattern('^(?:' + tokens[i].pattern + ')$')
        }
    }
 ...
 * @return {!RegExp}
 */
function attachKeys(re:Pattern<any>, keys:Key[]) {
    return Object.assign(re,{keys})
}
/**
...
function arrayToRegexp(path, keys, options) {
    ...
    const regexp = new Pattern('(?:' + parts.join('|') + ')', flags(options));
...
function tokensToRegExp(tokens, keys, options) {
    ...
    return attachKeys(new Pattern('^' + route, flags(options)), keys)
}
function pathToRegexp<T>(path:string|string[]|RegExp, keys:Key[], options:RegExpOptions={}):Pattern<T> {
